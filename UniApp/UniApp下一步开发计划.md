...
- ✅ 已实现 **AI 推荐点评展示**（调用后端推荐接口并展示推荐理由）
- ✅ 已实现点评列表展示（支持下拉刷新与上拉加载更多）
- ✅ 已实现创建点评入口按钮（未登录自动引导登录）
- ✅ 商家信息展示已补全（名称、分类、地址、评分、人均价格）
- ✅ 样式已优化为卡片式布局
- ⏳ 商家标签展示（如有标签数据）仍待接入

**创建点评页** (`pages/review/create.vue`):
- ✅ 已实现 **AI 草稿生成功能**（调用后端接口，带加载与错误提示）
- ✅ 已使用评分组件并优化基础样式
- ✅ 已增加内容字数统计与基础校验
- ⏳ 图片上传功能（选择、预览、上传）待实现

**首页** (`pages/home/index.vue`):
- ✅ 已实现分页加载功能（下拉刷新 + 上拉加载更多）
- ✅ 已使用商家卡片组件替换简单文本展示
- ⏳ 搜索功能仍缺少防抖处理
- ⏳ 样式可以进一步优化（搜索框、分类 Tab 等）

#### 2.2 公共组件（已补齐）✅

**components 目录与核心组件已创建完成：**
- ✅ `components/shop-card.vue` - 商家卡片组件
- ✅ `components/review-card.vue` - 点评卡片组件（支持 AI 推荐标识与推荐理由）
...
- [ ] 所有页面样式进一步优化（UI/布局/间距/字体层级）
- [ ] 列表加载骨架屏（可选）
- [ ] 错误提示更友好（统一 Toast/空态/重试入口）
- [ ] 空状态统一使用 `empty-state` 组件替换纯文本
- [ ] AI 功能加载与失败状态统一（可复用 `loading` 组件）
...
2. **点评卡片组件** (`components/review-card.vue`) ⭐⭐⭐ ✅
   ...
   - [x] 评分、内容、图片展示
   - [x] 点赞数、点赞按钮
   - [x] **AI 推荐标识**（如有推荐理由，显示特殊样式）
   - [x] **推荐理由展示**（浅蓝色背景、AI 图标）
   - [x] 时间格式化显示
...
**任务清单**:
- [x] **商家信息完善**
  - [x] 展示商家完整信息（名称、分类、地址、评分、人均价格）
  - [x] 使用商家卡片组件或优化展示样式
  - [x] 商家标签展示（如有）

- [x] **AI 推荐点评区域** ⭐⭐⭐（核心亮点）
  - [x] 调用 `recommendReviews` API
  - [x] 使用 review-card 组件展示推荐点评
  - [x] 推荐理由特殊样式（浅蓝色背景、AI 图标）
  - [x] 推荐理由可展开/收起（长文本时）
  - [x] 如果没有推荐结果，隐藏该区域
  - [ ] 加载状态和错误处理（使用通用 Loading 组件与错误提示）

- [x] **全部点评列表**
  - [x] 调用 `listReviews` API
  - [x] 分页加载（下拉刷新、上拉加载更多）
  - [x] 使用 review-card 组件展示
  - [x] 空状态提示（使用 `empty-state` 组件）

- [x] **创建点评入口**
  - [x] 底部固定按钮（或导航栏右侧按钮）
  - [x] 点击跳转 `pages/review/create?shopId=xxx`
  - [x] 未登录时提示登录
...
**任务清单**:
- [x] **表单完善**
  - [x] 使用 rating-stars 组件（可编辑模式）
  - [x] 内容输入框优化（多行、字数统计）
  - [x] 图片上传功能（最多 9 张）
    - [x] 使用 `uni.chooseImage` 选择图片
    - [x] 图片预览（可删除）
    - [x] 图片数据提交（JSON字符串格式，图片上传到服务器需后端支持）

- [x] **AI 草稿生成功能** ⭐⭐⭐（核心亮点）
  - [x] "AI 生成草稿"按钮（突出显示）
  - [x] 点击后调用 `generateAiDraft` API
  - [x] 显示加载状态（Loading 组件，显示"AI 正在生成中..."）
  - [x] 成功后填充到内容输入框
  - [x] 用户可编辑草稿内容
  - [x] 错误处理（生成失败提示）
  - [x] 可多次生成（覆盖或追加）
...
**任务清单**:
- [x] **商家列表优化**
  - [x] 使用 shop-card 组件替换当前简单展示
  - [x] 分页加载功能完善
    - [x] 实现上拉加载更多（onReachBottom）
    - [x] 分页状态管理（page、hasMore）
    - [x] 加载更多提示
...
2. **我的订单页优化** ✅
   - [x] 使用订单卡片组件（已创建 `order-card.vue`）
   - [x] 订单信息展示优化（商家名称、金额、时间、消费项明细）
   - [x] 空状态提示（使用 `empty-state` 组件）
   - [x] 统一加载状态（使用 `loading` 组件）
...
4. **偏好设置页优化** ✅
   - [x] 标签选择器样式优化（渐变背景、选中动画）
   - [x] 保存成功提示优化（加载状态、错误处理）
   - [x] 统一加载和空状态组件
...
- [x] 开发 `components/review-card.vue`（含 AI 推荐标识）
- [x] 开发 `components/rating-stars.vue`
- [x] 开发 `components/loading.vue`
- [x] 开发 `components/empty-state.vue`
- [x] 开发 `components/ai-recommend-badge.vue`
